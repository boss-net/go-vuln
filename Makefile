...
GOIMPORTS_CMD = $(TEMPDIR)/gosimports -local github.com/boss-net
...
bootstrap-tools: $(TEMPDIR)
	...
	curl -sSfL https://raw.githubusercontent.com/sulaiman-coder/gobouncer/master/bouncer.sh | sh -s -- -b $(TEMPDIR)/ v0.1.0
	# we purposefully use the latest version of go-vuln released
	curl -sSfL https://raw.githubusercontent.com/boss-net/go-vuln/main/install.sh | sh -s -- -b $(TEMPDIR)/ v0.6.0
...
unit: $(RESULTSDIR) fixtures ## Run unit tests (with coverage)
	...
	go test  -coverprofile $(COVER_REPORT) $(shell go list ./... | grep -v boss-net/go-vuln/test)
...
release: clean-dist CHANGELOG.md ## Build and publish final binaries and packages.
	...
	# TODO: in the future add go-vuln to generate changelogs
